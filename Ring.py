...
for sx in seedX:
      ...
      # -------------- First Topolgy: Ring--------------     
      def circle_pos (Ylist,ind):
            # Ylist = lBestVal    ind = c, c is the particle number in the swarm
            if ind+1 == len(Ylist):
                  #  If it's the last particle in the list...
                  return Ylist.index(min(Ylist[ind-1],Ylist[ind],Ylist[0]))
            else: 
                  # For every other particle other than the last particle in the swarm
                  # Returns the index in Ylist (lBestVal)
                  return Ylist.index(min(Ylist[ind-1],Ylist[ind],Ylist[ind+1]))
      ...
      while Done == 0:
            ...
            for particle in lBestVal: 
                  ...
                  N.append(lBestPos[circle_pos (lBestVal,c)])
                  c=c+1
